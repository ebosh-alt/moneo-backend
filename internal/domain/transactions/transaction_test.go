...
func TestNewTransactionValidatesTypeSpecificInvariants(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "transfer does not allow category",
			params: NewTransactionParams{
				ID:            shared.TransactionID("tx-1"),
				UserID:        shared.UserID("user-1"),
				Type:          TransactionTypeTransfer,
			...
			want: ErrTransactionCategoryMustBeEmpty,
		},
		{
			name: "transfer does not allow subcategory",
			params: NewTransactionParams{
				ID:            shared.TransactionID("tx-1"),
				UserID:        shared.UserID("user-1"),
				Type:          TransactionTypeTransfer,
				...
				AccountFromID: &from,
				AccountToID:   &to,
				SubcategoryID: func() *shared.SubcategoryID { v := shared.SubcategoryID("sub-1"); return &v }(),
				CreatedAt:     now,
				UpdatedAt:     now,
			},
			want: ErrTransactionSubcategoryMustBeEmpty,
		},
		{
		...
		{
			name: "investment does not allow account_to",
			params: NewTransactionParams{
				ID:            shared.TransactionID("tx-1"),
				UserID:        shared.UserID("user-1"),
				Type:          TransactionTypeInvestment,
